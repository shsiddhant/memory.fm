...
year = 2024
min_length = 3
from_ts = datetime.datetime.fromtimestamp(1707419854, tz=datetime.timezone.utc)


def test_get_streaks_by_username(seeded_db: Session):
    streaks = strkserv.get_streaks_by_username(seeded_db, username, kind, from_ts)
    assert streaks is not None
    assert len(streaks) == 2, repr(streaks)  # Show value on failure
